...
class MigratePlugin(commands.Cog):
    ...
    async def migrate_cmd(self, ctx, token: str = None):
        ...
        if errors:
            ...
            restart_note = " The bot has been restarted to apply the new config." if restarted else " Restart the bot from the Avionico dashboard to apply the new config."
            await msg.edit(
                content=f"Done. {total} documents pushed to Avionico successfully.{restart_note}"
            )
